...
import { contentApi, photoApi } from "~/services/api";
...
export const Hero = component$(() => {
    ...
    contentApi.getContentByCode(slideshowParentCode).then((parentData) => {
      if (!parentData) {
        ...
        contentApi.createContentCustom({
          ...
          console.log(`Created New content for ${parentData}`, newData);
  ...
  return (
    <>
      <div class="fixed top-24 left-4 z-30">
        {/* Consolidated Slideshow Controls Button */}
        <div class="relative group">
          {/* Main Button */}
          <button class="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg shadow-lg hover:shadow-blue-500/50 transition-all duration-300 hover:scale-105 flex items-center gap-2">
            <span>⚙️</span>
            <span>Slideshow</span>
          </button>
          ...
          <div class="absolute left-full ml-2 top-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
            <div class="bg-white rounded-lg shadow-xl border border-gray-200 min-w-[200px]">
              ...
              {/* Edit Slideshow Option */}
              <ModalForm
                formData={formData}
                heroIsPlaying={heroIsPlaying}
              >
                <div class="w-full px-4 py-3 text-left flex items-center gap-2 text-gray-700 hover:bg-gray-50 cursor-pointer">
                  <span>✏️</span>
      ...
      >
        {/* Slideshow Backgrounds */}
        {
          heroSlides.map((slide, idx) => (
            <div
              key={slide.code}
              class={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 z-0 ${heroCurrentSlide.value === idx ? 'opacity-100' : 'opacity-0'
                }`}
        ...
        {/* Dark Overlay */}
        <div class="absolute inset-0 bg-gradient-to-b from-black/30 to-black/50 z-[1]" />
        ...
        <div class="container mx-auto px-6 relative z-10">
          <div class="text-center text-white">
            ...
            <div class="flex justify-center gap-4 mt-8 flex-wrap">
              <div class="relative inline-block group">
                <a href="#features" class="bg-white text-purple-600 px-8 py-3 rounded-full font-semibold hover:bg-gray-100 transition duration-300 shadow-lg inline-block">
                  Get Started
                </a>
        ...
        {/* Slideshow Controls */}
        <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-4 items-center">
          <div class="relative group">
            <button onClick$={heroPrevSlide} class="bg-white/20 backdrop-blur-lg border border-white/30 text-white px-4 py-2 rounded-lg hover:bg-white/30 transition">
              ❮ Prev
            </button>
            <button class="absolute -top-2 -left-2 bg-white text-purple-600 px-2 py-1 rounded shadow-lg text-xs font-semibold hover:bg-gray-100 opacity-0 group-hover:opacity-100 transition-all duration-300">
              ⬅️
            </button>
          </div>
          ...
          <div class="relative group">
            <button onClick$={heroTogglePlayPause} class="bg-white/20 backdrop-blur-lg border border-white/30 text-white px-4 py-2 rounded-lg hover:bg-white/30 transition">
              {heroIsPlaying.value ? '⏸ Pause' : '▶ Play'}
            </button>
            <button class="absolute -top-2 -right-2 bg-yellow-500 text-white px-2 py-1 rounded shadow-lg text-xs font-semibold hover:bg-yellow-600 opacity-0 group-hover:opacity-100 transition-all duration-300">
              ⚙️
            </button>
          </div>

          <div class="relative group">
            <button onClick$={heroNextSlide} class="bg-white/20 backdrop-blur-lg border border-white/30 text-white px-4 py-2 rounded-lg hover:bg-white/30 transition">
              Next ❯
            </button>
            <button class="absolute -top-2 -right-2 bg-white text-purple-600 px-2 py-1 rounded shadow-lg text-xs font-semibold hover:bg-gray-100 opacity-0 group-hover:opacity-100 transition-all duration-300">
              ➡️
            </button>
          </div>
        </div>
