import { component$, useStore, useSignal, useVisibleTask$, useComputed$ } from "@builder.io/qwik";
import { ModalForm } from "~/components/modalForm";
import { contentApi, OptValueType } from "~/services/api";

// Supported platforms and their SVG paths
...
export const SocialLinks = component$(() => {
  const code = useSignal("Kp7mNzQwR2xYtLsJ4vHdAeUfCgMbOiXn5Vl9qPk8WrTaZcEyGhDjFuB"); // social-links-01
  const template = useSignal(1);

  const formData = useStore({
    modalCode: code.value,
    modalType: OptValueType.TEXT,
    modalTitle: "Social Links (JSON)",
    modalWidth: String(0),
    modalHeight: String(0),
    value_text: "",
    value_textarea: JSON.stringify(DEFAULT_LINKS, null, 2),
    value_image: "",
    processedFileName: "",
    imagePreview: "",
  });
    ...
    contentApi.getContentByCode(code.value).then((data) => {
      if (!data) {
        contentApi.createContentCustom({
          ...
          code: code.value,
          value_type: OptValueType.TEXT,
          value_text: "Social Links",
          value_textarea: JSON.stringify(DEFAULT_LINKS),
        }).then((newData) => {
        ...
        formData.modalTitle = data.value_text || "Social Links (JSON)";
        formData.value_text = data.value_text || "";
        formData.value_textarea = data.value_textarea || JSON.stringify(DEFAULT_LINKS);
      }
    });
  });

  const links = useComputed$<SocialLinkItem[]>(() => {
    try {
      const parsed = JSON.parse(formData.value_textarea || "[]");
    ...
    <ModalForm formData={formData}>
      <div class="flex items-center gap-4">
        {links.value.map((item) => {
          const iconPath = PLATFORM_ICONS[item.platform.toLowerCase()];
